...
async function ESBuildLoader(source) {
  ...
  if (!("tsconfigRaw" in transformOptions)) {
    ...
    if (tsconfigPath) {
      ...
      const tsconfig = getTsconfig.getTsconfig(resourcePath);
      if (tsconfig) {
...
function EsbuildPlugin({
  ...
  const pluginInstance = {
    apply(compiler) {
      ...
      const usedAsMinimizer = (_c = (_b = (_a2 = compiler.options.optimization) == null ? void 0 : _a2.minimizer) == null ? void 0 : _b.includes) == null ? void 0 : _c.call(_b, pluginInstance);
      if (usedAsMinimizer && !("minify" in options || "minifyWhitespace" in options || "minifyIdentifiers" in options || "minifySyntax" in options)) {
        options.minify = (_d = compiler.options.optimization) == null ? void 0 : _d.minimize;
      }
